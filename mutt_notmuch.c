...
bool nm_normalize_uri(char *new_uri, const char *orig_uri, size_t new_uri_sz)
{
  ...
  tmp_ctxdata.db_url_holder = safe_strdup(orig_uri);

  if (url_parse_with_qs(&tmp_ctxdata.db_url, tmp_ctxdata.db_url_holder) < 0)
  {
    rc = -1;
    goto gone;
  }
...
gone:
  ...
  if (rc < 0)
  {
    ...
    mutt_debug(2, "nm_normalize_uri () -> error #%d\n", -rc);
    return false;
  }
  return true;
}
